...
const ResourceTrading = () => {
    ...
    const fetchResources = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/resource/all`);
    ...
    const handleEditResource = (resource) => {
        if (user?.email && resource.ownerEmail && user.email.toLowerCase() === resource.ownerEmail.toLowerCase()) {
                ...
                {filteredResources.map((resource) => (
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex justify-center items-center gap-3 text-white">
                                <div>
                                    Quantity: <span className="italic">{resource.quantity}</span>
                                </div>
                                |
                                <div>
                                    Trade Type: <span className="italic">{resource.tradeType}</span>
                                </div>
                            </div>
                        </div>

                        <div className="flex items-center justify-center gap-5">
                            {user?.email && resource.ownerEmail && user.email.toLowerCase() === resource.ownerEmail.toLowerCase() && (
                                <PencilIcon
